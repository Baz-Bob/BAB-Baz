...
description: "用 Boltz 筛选现有小分子库。适用于将给定的 SMILES 或化合物库针对靶点进行对接、打分或排序的场景；同时免费返回每个分子的一级 ADME/ADMET（溶解度、渗透性、亲脂性/logD）。不适用于从头分子设计、一次性对接，或对无靶点的裸 SMILES 做 ADME（请用 boltz-small-molecule-adme）。"
---

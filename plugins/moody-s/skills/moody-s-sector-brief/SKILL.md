...
description: "使用 Moody's GenAI MCP 工具及网络调研，为任意行业生成一份行业简报 HTML 报告。当用户要求分析某个行业、撰写行业报告、开展行业分析、创建行业概览或生成行业深度解读时使用本技能。即使只是点名行业并提到「analysis」「overview」「outlook」「report」或「deep-dive」也应触发。对于「零售行业正在发生什么」「给我一份航空航天行业的拆解」等说法也会触发。"
---

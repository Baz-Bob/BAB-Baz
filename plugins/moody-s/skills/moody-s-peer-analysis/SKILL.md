...
description: "使用 Moody's GenAI MCP 工具，为目标公司及其信用同业生成一份同业分析 HTML 报告。当用户要求将某公司与其同业对比、开展同业分析、进行信用同业比较、生成同业组合报告，或分析相对信用定位时使用本技能。即使只是点名公司并提到「peers」「peer comparison」「credit comparison」「peer group」或「relative value」也应触发。"
---

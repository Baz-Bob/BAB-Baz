...
description: "使用 Moody's GenAI MCP 工具为某公司生成一份评级推介报告，以独立 HTML 文件形式保存到磁盘。当用户要求创建评级推介、评级推介演示文稿、信用推介、评级演示、评级推介报告或评级 HTML 报告时使用本技能。当用户要求将行业分析、公司财务、SWOT、同业对比及 ESG 合并为单一报告或演示的全面信用概览时，也应触发。即使只是点名公司并说「pitch deck」「rating deck」「credit deck」或「rating report」也应触发。"
---

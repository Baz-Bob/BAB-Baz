...
description: "使用 Moody's GenAI MCP 工具，为某公司生成一份全面的 Issuer Brief HTML 报告。当用户要求创建 Issuer Brief、公司档案报告、信用备忘录、投资手册或全面公司分析时使用本技能。当用户要求将公司概览、财务数据、同业对比、行业概览、战略动态、管理层、信用画像、风险及 ESG 合并为单一文档的报告时，也应触发。即使只是点名公司并说「Issuer Brief」「info book」「company book」「credit book」或「full company report」也应触发。"
---

---
name: render-mcp
description: "为 AI 编码工具连接并配置 Render MCP server——按工具配置(Cursor、Claude Code、Codex)、认证、工作区选择、工具目录及排障。当 MCP 未配置、list_services() 失败、用户询问 Render MCP 配置,或某个操作类技能需要 MCP 但尚未连接时使用。触发词:MCP、Render MCP、list_services、MCP setup、MCP server、API key、Bearer token、mcp.render.com、workspace selection。"
license: MIT

---
name: render-domains
description: "在 Render 上配置自定义域名与 TLS 证书——DNS 设置、CNAME 记录、apex 域名、泛域名及证书排障。当用户需要添加自定义域名、配置 DNS、设置 HTTPS/TLS、排查证书签发、停用 onrender.com 子域名,或添加泛域名时使用。触发词:custom domain、DNS、CNAME、TLS、SSL、HTTPS、certificate、apex domain、wildcard domain、onrender.com、domain verification。"
license: MIT

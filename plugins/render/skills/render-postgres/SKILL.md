...
description: "在 Render 上搭建并优化托管 PostgreSQL——连接串(内网与外网)、创建约束、存储自动扩容、连接数上限、高可用、只读副本、备份及 MCP 检查。当用户提到 Postgres、PostgreSQL、Render 数据库、连接串、DATABASE_URL、备份、快照、副本、HA、磁盘存储、连接池,或排查数据库连通性时使用。"
license: MIT

...
description: "为 Render 基础设施编写并校验 render.yaml Blueprint。当用户需要编写或编辑 render.yaml、用 fromDatabase/fromService/fromGroup 串联服务、为多服务应用设置 project 与 environment、配置预览环境、对照 schema 校验,或修复不可变字段报错时使用。触发词:render.yaml、Blueprint、IaC、fromDatabase、fromService、envVarGroups、previews、projects、environments。"
license: MIT
compatibility: >-

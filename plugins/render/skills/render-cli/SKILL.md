---
name: render-cli
description: "安装并使用 Render CLI 进行部署、查看日志、SSH、psql、Blueprint 校验与自动化。当用户需要运行 Render CLI 命令、在 CI/CD 中脚本化部署、用 API key 认证、非交互式查询服务,或排查 CLI 认证问题时使用。触发词:render CLI、render login、render deploys、render logs、render ssh、render psql、render blueprints validate、render skills、RENDER_API_KEY、non-interactive、CI/CD deploy。"
license: MIT

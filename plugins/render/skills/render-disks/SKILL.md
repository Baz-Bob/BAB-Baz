---
name: render-disks
description: "为 Render 服务挂载并管理持久化磁盘——挂载路径、容量、快照、文件传输及单实例约束。当用户需要持久化存储、文件上传、磁盘上的自建数据库、CMS 媒体存储,或需要理解为何服务无法横向扩展或使用零停机部署时使用。触发词:persistent disk、disk、storage、mount path、sizeGB、SSD、file uploads、snapshots、disk restore、ephemeral filesystem。"
license: MIT

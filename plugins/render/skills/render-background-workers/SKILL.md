...
description: "在 Render 上搭建并配置后台 worker,用于基于队列的任务处理。当用户需要处理异步任务、从队列消费、运行 Celery/Sidekiq/BullMQ/Asynq/Oban worker、用 SIGTERM 实现优雅关闭、将 worker 接入 Key Value(Redis),或在 worker 与 cron 任务之间做选择时使用。触发词:background worker、async jobs、queue consumer、Celery、Sidekiq、BullMQ、Asynq、Oban、job processing、SIGTERM、graceful shutdown。"
license: MIT

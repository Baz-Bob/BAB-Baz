...
description: "预置并配置 Render Key Value(兼容 Redis 的 Valkey 8)实例,用于缓存、会话存储与任务队列。当用户需要 Redis、Key Value、Valkey、缓存、会话存储、任务队列后端,或需要配置 maxmemory 策略、ipAllowList、连接串,或内网与外网访问时使用。触发词:Key Value、Redis、Valkey、cache、session store、REDIS_URL、maxmemory、ipAllowList、allkeys-lru、noeviction。"
license: MIT

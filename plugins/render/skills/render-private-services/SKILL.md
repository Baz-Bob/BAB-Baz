...
description: "配置 Render private service——仅内部可用、只通过私有网络接收来自其它 Render 服务流量的应用。当用户需要内部 API、微服务、gRPC server、sidecar,或任何不应对公网暴露的服务时使用。在 private service 与后台 worker 之间做选择时也适用。触发词:private service、pserv、internal service、internal API、microservice、gRPC、not public、private network service。"
license: MIT

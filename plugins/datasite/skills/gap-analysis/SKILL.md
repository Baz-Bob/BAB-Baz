---
name: gap-analysis
description: "面向 Datasite 交易室的数据室缺口分析(Gap Analysis)技能。当卖方交易团队想在对买方开放前审核数据室中有哪些缺失、稀疏或不完整之处时使用本技能。触发场景包括:「做缺口分析」「数据室还缺什么」「检查数据室覆盖度」「标出空文件夹」「我们还没上传什么」「数据室就绪检查」「上线前找出缺口」「合同是否都在里面」「检查我们是否齐全」,或任何按章节评估数据室完整度的请求。每当交易团队准备启动数据室、想知道还需上传或整理什么时,应主动使用本技能。不要用于 PII、脱敏等文档质量问题(用 document-quality-check),也不要用于草拟 Q&A 回复(用 bulk-qa-answers)。"
metadata:
  author: Blueflame AI
  version: 1.0.0
  mcp-server: datasite

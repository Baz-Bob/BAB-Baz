...
description: "面向 Datasite 交易室的文档质量检查技能。当交易团队想在对买方开放前审核文档质量时使用本技能。触发场景包括:「检查文档质量」「标出有问题的文档」「找出受密码保护的文件」「检查空白文档」「PII 检查」「脱敏审查」「找出损坏的文件」「文档审计」「质检数据室」「有没有空白或损坏的文件」「检查未脱敏的个人数据」,或任何核实数据室文档是否完整、可访问、可安全共享的请求。应在数据室上线前主动使用本技能。不要用于文件重命名(用 smart-file-renaming)或识别缺失章节(用 gap-analysis)。"
metadata:
  author: Blueflame AI
  version: 1.0.0
  mcp-server: datasite

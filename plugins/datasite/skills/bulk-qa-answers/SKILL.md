...
description: "面向 Datasite 交易室的批量 Q&A 作答技能。当卖方交易团队想一次性回答多个买方问题、根据 VDR 内容生成 AI 草拟回复、产出 Q&A 跟踪表,或搭建 Q&A 管理仪表盘时使用本技能。触发场景包括:「回答 Q&A」「草拟对买方问题的回复」「处理问题清单」「生成 Q&A 跟踪表」「回答所有问题」「批量作答」「Q&A 管理仪表盘」「回应尽调问题」,或任何以数据室内容为来源、系统性逐条处理买方问题清单的请求。每当买方提交了问题、交易团队希望 AI 辅助草拟时,应主动使用本技能。不要用于结构化 Q&A 流程之外的单个零散问题。不要凭通用常识草拟答案——所有回复必须来自数据室。"
metadata:
  author: Blueflame AI
  version: 1.0.0
  mcp-server: datasite

...
description: "面向 Datasite 交易室的 VDR 索引搭建技能。当用户想为一笔交易创建、提议、设计或搭建虚拟数据室(VDR)索引或文件夹结构时使用本技能。触发场景包括:「搭建数据室」「创建 VDR 索引」「搭建交易室结构」「准备索引」「搭建文件室」「我需要为某笔交易/某家公司建一个数据室」,或任何为尽职调查组织或构建文档结构的请求。当用户想复制现有交易室结构,或从电子表格或参考交易导入索引时也会触发。每当用户开始一个新交易室,或想在上传文档前自定义文件夹层级时,必须使用本技能。不要用于审计或审查现有数据室——那用 gap-analysis、document-quality-check 或 risk-analysis-audit。"
metadata:
  author: Blueflame AI
  version: 1.0.0
  mcp-server: datasite

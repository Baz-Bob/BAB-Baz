---
name: irl-tracker
description: "面向 Datasite 交易室的信息请求清单(IRL)跟踪技能。当交易团队想将 VDR 内容与买方的信息请求清单比对、跟踪文档交付状态,或搭建尽职调查跟踪仪表盘时使用本技能。触发场景包括:「映射 IRL」「跟踪已提供内容」「检查信息请求清单」「信息收集清单」「IGL」「我们交付了什么」「DD 跟踪表」「尽调跟踪表」「将 VDR 与请求清单比对」「还有什么未完成」「搭建尽调仪表盘」,或任何对照买方请求跟踪文档交付的请求。每当买方提交了请求清单、交易团队需要管理并跟踪回复时,主动使用。不要用于数据室整体结构性缺口分析——那用 gap-analysis。"
metadata:
  author: Blueflame AI
  version: 1.0.0
  mcp-server: datasite

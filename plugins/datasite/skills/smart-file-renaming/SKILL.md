...
description: "面向 Datasite 交易室的智能文件重命名技能。当交易团队想在上线前规范文档命名、整理扫描件文件名、统一同类文档的命名,或提升数据室的专业度时使用本技能。触发场景包括:「重命名文件」「整理文件名」「统一命名」「文件名一团乱」「修正文档名」「重命名扫描件」「让命名保持一致」「整理数据室」,或任何改进、清理或规范 Datasite 项目中文档命名的请求。未经用户明确确认,绝不执行任何重命名。不要用于文档质量或 PII 检查——那用 document-quality-check。未经用户明确确认,绝不重命名文件。"
metadata:
  author: Blueflame AI
  version: 1.0.0
  mcp-server: datasite

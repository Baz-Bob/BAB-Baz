...
description: "面向 Datasite 交易室的上线就绪编排(Launch Readiness Orchestrator)技能。当交易团队想在上线前对数据室做一次性的整体就绪检查——把缺口分析、文档质量审计与风险审查合并为一个「数据室是否就绪?」的整合视图时使用本技能。触发场景包括:「我们可以上线了吗」「上线就绪检查」「上线前审计」「数据室就绪度」「能上线吗」「数据室准备好了吗」「跑一次完整就绪检查」「上线检查清单」「上线前清单」,或任何在对买方开放数据室前获取单一整体评估的请求。每当交易团队临近上线日、想要一份结构化的签核视图时,主动使用。本技能激活时,不要再使用其它单项审计技能(gap-analysis、document-quality-check、risk-analysis-audit)——本技能会在一轮内编排这三者。"
metadata:
  author: Blueflame AI
  version: 1.0.0
  mcp-server: datasite

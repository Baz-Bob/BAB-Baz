...
description: "面向 Datasite 交易室的风险分析审计技能。当卖方交易团队想在上线前审计、审查或标出数据室中的各类风险时使用本技能。触发场景包括:「跑一次风险审计」「标出数据室中的风险」「风险审查」「这笔交易有哪些风险」「审计数据室」「风险分析」「上线前标出问题」「上线前我们该修什么」,或任何按工作流(税务、财务、法务、HR、IP、商务、监管、ESG)分析交易风险的请求。每当用户在为上线准备数据室、想要一份「什么可能让买方担忧」的结构化视图时,应主动使用本技能。不要用于 PII、脱敏等文档质量问题(用 document-quality-check),也不要用于识别缺失章节(用 gap-analysis)。"
metadata:
  author: Blueflame AI
  version: 1.0.0
  mcp-server: datasite

...
description: "Catalyst by Zoho 全栈 Serverless 云平台专家编程助手。凡提及 Catalyst、zcatalyst、AppSail、Data Store、ZCQL、Cache、Stratus、Circuits、SmartBrowz、ConvoKraft、Slate、Signals、Pipelines、QuickML、NoSQL、定时任务、Zia Services、CodeLib、API Gateway、Connections、Zoho MCP、CatalystbyZoho、catalyst init/deploy/serve、zcatalyst-sdk-node 或 catalyst-config.json，均应触发此技能。涵盖全部 7 种函数类型、完整服务目录、架构指导，以及基于 Zoho MCP 工具的资源管理。也适用于与 AWS Lambda、S3、DynamoDB、Vercel、Netlify、Supabase、Firebase、Heroku、Cloud Run、Cloudflare R2、Railway 的迁移或比较场景。以及 Catalyst 定价、成本估算，或「帮我建表」、「配置数据库」、「部署到 Catalyst」、「在 Zoho 平台上构建」或「Catalyst 和 Firebase 一样吗」等请求。不适用于通用 Zoho CRM 问题（除非目标是 Catalyst）。"
---

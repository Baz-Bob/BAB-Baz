...
description: "当用户要求对整个仓库或特定路径进行深度、穷尽、多轮或降方差的 Codex Security 扫描时使用。在同一个已确定的范围上,用各 worker 专属的威胁模型反复进行独立的发现(discovery)轮次,在语义层面合并候选项,综合出一份规范的验证威胁模型,然后一次性运行验证、攻击路径分析、规范 JSON 补全及报告生成。不要用于 PR、提交、分支 diff 或工作区 diff。"
---

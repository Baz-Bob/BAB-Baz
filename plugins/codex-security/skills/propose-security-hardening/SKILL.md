...
description: "基于漏洞披露、提供的发现项、事件或评估文档、源代码，或已完成的 Codex Security 扫描，制定有据可循的结构性与架构性安全加固方案。当用户要求系统性改进、超越单点修复的替代方案、加固前后的安全架构视图、工程权衡分析，或针对选定加固选项的可实施计划时使用。当顶层扫描工作流请求最终报告的加固指导时，也会在带有可报告发现项的 Codex Security 扫描之后自动使用。"
---

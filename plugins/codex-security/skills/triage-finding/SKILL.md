---
name: triage-finding
description: "当用户从扫描器、安全公告、GitHub、Atlassian Rovo、Linear 或类似 backlog 来源提供或导入已有的安全发现项、漏洞报告,或安全/漏洞类 Jira/Linear 工单,并希望对仓库做静态影响分诊时使用。不要用于发现(discovery)、重复 bug 分诊、验证或修复。"
metadata:

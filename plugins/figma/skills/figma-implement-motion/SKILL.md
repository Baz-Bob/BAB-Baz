...
description: "将 Figma 的动效与动画转译为生产级应用代码。在从 Figma 设计实现动画/动效时使用--用户提到「implement this motion」「add animation from Figma」「animate this component」、提供了一个节点带动画的 Figma URL，或当 `get_design_context` 返回动效数据/指示你调用 `get_motion_context` 时触发。"

...
description: "在将 Figma 设计实现为代码（design -> code，即从 Figma 读取的方向）时使用本技能。触发词：「implement this Figma design」「build this screen from Figma」「turn this Figma into code」「code this up from Figma」「design to code」，或在代码库旁提供了 figma.com 设计 URL 时触发。封装了以下工作流：用 get_design_context 从 Figma 读取节点，并将其引用输出适配到目标项目--复用既有组件、token 与约定，并按优先级遵循 Code Connect 映射、组件文档、设计标注及设计 token。与 figma-code-connect（组件映射）以及 figma-generate-design / figma-use（反向，code -> design 方向）互补。"

---
name: figma-swiftui
description: "SwiftUI ↔ Figma 双向转译。当用户提到 Swift、SwiftUI、iOS、iPhone 或 iPad 时使用--支持任一方向：将 Figma 设计转译为 SwiftUI（design -> code），或将 SwiftUI 视图/屏幕/token 推回到 Figma 文件中（code -> design）。触发词如「implement this Figma design in SwiftUI」「build this screen in Swift」「push this SwiftUI view to Figma」「mirror my Swift code in a Figma file」，或当 Figma URL 与 `.swift` 文件 / `.xcodeproj` 同时出现时触发。会路由到方向专属的参考文档；在 code -> design 路径下与 `figma-use` 一起加载。"
